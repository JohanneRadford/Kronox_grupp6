				...
				if(p.getNodeType() == Node.ELEMENT_NODE){
		            ...
		            for (int t = 0; t < items3.getLength(); t++)
		            {
		                ...
		                if (n1.getNodeType() == Node.ELEMENT_NODE) {
		        			...
		        			info = eElement.getTextContent().replace("&#246;", "ö").replace("&#214;", "Ö").replace("&#228;", "ä").replace("&#229;", "å").replace("<br>", " ").replace("&#822;1", " ").replace("&#233;","è").replace("<br />", " ").replace("&#196;", "Ä").replace("<strong>", " ").replace("</strong>", " ").replace("&#8217;", "´").replace("&#8216", "`").replace("&amp;", "&").replace("&#197;", "Å");
